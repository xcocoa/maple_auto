...
public final class C$Gson$Types {
    ...
    public static final class GenericArrayTypeImpl implements GenericArrayType, Serializable {
        ...
        public String toString() {
            return C$Gson$Types.typeToString(this.componentType) + kx.OooOOO;
        }
    }
        ...
        while (true) {
            if (type2 instanceof TypeVariable) {
                ...
                if (cls2.isArray()) {
                    ...
                    typeResolve = resolve(type, cls, componentType, map);
                    ...
                    typeResolve = resolve(type, cls, genericComponentType, map);
                } else {
                    if (type2 instanceof ParameterizedType) {
                        type2 = (ParameterizedType) type2;
                        Type ownerType = type2.getOwnerType();
                        Type typeResolve2 = resolve(type, cls, ownerType, map);
                        boolean z = !equal(typeResolve2, ownerType);
                        Type[] actualTypeArguments = type2.getActualTypeArguments();
                        int length = actualTypeArguments.length;
                        for (int i = 0; i < length; i++) {
                            Type typeResolve3 = resolve(type, cls, actualTypeArguments[i], map);
                            if (!equal(typeResolve3, actualTypeArguments[i])) {
                                if (!z) {
                                    actualTypeArguments = (Type[]) actualTypeArguments.clone();
                                    z = true;
                                }
                                actualTypeArguments[i] = typeResolve3;
                            }
                        }
                        if (z) {
                            typeNewParameterizedTypeWithOwner = newParameterizedTypeWithOwner(typeResolve2, type2.getRawType(), actualTypeArguments);
                        }
                        ...
                        if (lowerBounds.length == 1) {
                            Type typeResolve4 = resolve(type, cls, lowerBounds[0], map);
                            if (typeResolve4 != lowerBounds[0]) {
                                type2 = supertypeOf(typeResolve4);
                            }
                        } else if (upperBounds.length == 1) {
                            Type typeResolve5 = resolve(type, cls, upperBounds[0], map);
                            if (typeResolve5 != upperBounds[0]) {
                                type2 = subtypeOf(typeResolve5);
                            }
                        }
                    }
                }
