...
public class IMAPProtocol extends Protocol {
    ...
    public IMAPProtocol(String str, String str2, int i, Properties properties, boolean z, MailLogger mailLogger) throws ProtocolException, IOException {
        super(str2, i, properties, "mail.".concat(String.valueOf(str)), z, mailLogger);
        this.connected = false;
        this.rev1 = false;
        this.noauthdebug = true;
        try {
    ...
    private void checkReferral(Response response) throws IMAPReferralException {
        ...
        if (rest.startsWith("[") && (iIndexOf = rest.indexOf(32)) > 0 && rest.substring(1, iIndexOf).equalsIgnoreCase("REFERRAL")) {
            ...
            int i = iIndexOf + 1;
            if (iIndexOf2 > 0) {
                strSubstring = rest.substring(i, iIndexOf2);
                strTrim = rest.substring(iIndexOf2 + 1).trim();
            } else {
                strSubstring = rest.substring(i);
                strTrim = "";
            }
            if (response.isBYE()) {
                disconnect();
    ...
    private int[] issueSearch(String str, SearchTerm searchTerm, String str2) throws SearchException, ProtocolException, IOException {
        ...
        Response[] responseArrCommand = str2 == null ? command("SEARCH", argumentGenerateSequence) : command("SEARCH CHARSET ".concat(String.valueOf(str2)), argumentGenerateSequence);
        Response response = responseArrCommand[responseArrCommand.length - 1];
        if (response.isOK()) {
            ArrayList arrayList = new ArrayList();
            int length = responseArrCommand.length;
            for (int i = 0; i < length; i++) {
                if (responseArrCommand[i] instanceof IMAPResponse) {
                    IMAPResponse iMAPResponse = (IMAPResponse) responseArrCommand[i];
    ...
    private void storeFlags(String str, Flags flags, boolean z) throws ProtocolException {
        ...
        Response[] responseArrCommand = command(sb.toString(), null);
        notifyResponseHandlers(responseArrCommand);
        handleResult(responseArrCommand[responseArrCommand.length - 1]);
    }
    ...
    public synchronized void authlogin(String str, String str2) throws ProtocolException {
        ...
        try {
            ...
            if (this.noauthdebug && isTracing()) {
                this.logger.fine("AUTHENTICATE LOGIN command result: ".concat(String.valueOf(responseByeResponse)));
            }
    ...
    public synchronized void authntlm(String str, String str2, String str3) throws ProtocolException {
        ...
        try {
            ...
            notifyResponseHandlers(responseArr);
            if (this.noauthdebug && isTracing()) {
                this.logger.fine("AUTHENTICATE NTLM command result: ".concat(String.valueOf(responseByeResponse)));
            }
            handleLoginResult(responseByeResponse);
            setCapabilities(responseByeResponse);
            this.authenticated = true;
    ...
    public synchronized void authoauth2(String str, String str2) throws ProtocolException {
        ...
        try {
            ...
            notifyResponseHandlers(responseArr);
            if (this.noauthdebug && isTracing()) {
                this.logger.fine("AUTHENTICATE XOAUTH2 command result: ".concat(String.valueOf(responseByeResponse)));
            }
            handleLoginResult(responseByeResponse);
            setCapabilities(responseByeResponse);
            this.authenticated = true;
    ...
    public synchronized void authplain(String str, String str2, String str3) throws ProtocolException {
        ...
        try {
            ...
            notifyResponseHandlers(responseArr);
            if (this.noauthdebug && isTracing()) {
                this.logger.fine("AUTHENTICATE PLAIN command result: ".concat(String.valueOf(responseByeResponse)));
            }
            handleLoginResult(responseByeResponse);
            setCapabilities(responseByeResponse);
            this.authenticated = true;
    ...
    public MailboxInfo examine(String str, ResyncData resyncData) throws ProtocolException {
        ...
        if (resyncData != null) {
            if (resyncData == ResyncData.CONDSTORE) {
                ...
                argumentResyncArgs = new Argument().writeAtom("CONDSTORE");
            } else {
                if (!hasCapability("QRESYNC")) {
                    throw new BadCommandException("QRESYNC not supported");
                }
                argumentResyncArgs = resyncArgs(resyncData);
            }
            argument.writeArgument(argumentResyncArgs);
        }
    ...
    public Flags fetchFlags(int i) throws ProtocolException {
        ...
        while (true) {
            if (i2 < length) {
                if (responseArrFetch[i2] != null && (responseArrFetch[i2] instanceof FetchResponse) && ((FetchResponse) responseArrFetch[i2]).getNumber() == i && (flags = (Flags) ((FetchResponse) responseArrFetch[i2]).getItem(FLAGS.class)) != null) {
    ...
    public RFC822DATA fetchRFC822(int i, String str) throws ProtocolException {
        Response[] responseArrFetch = fetch(i, str == null ? "RFC822" : "RFC822.".concat(String.valueOf(str)));
        notifyResponseHandlers(responseArrFetch);
        Response response = responseArrFetch[responseArrFetch.length - 1];
        if (response.isOK()) {
    ...
    public long[] fetchSequenceNumbers(long j, long j2) throws ProtocolException {
        ...
        int length = responseArrFetch.length;
        for (int i = 0; i < length; i++) {
            if (responseArrFetch[i] != null && (responseArrFetch[i] instanceof FetchResponse) && (uid = (UID) ((FetchResponse) responseArrFetch[i]).getItem(UID.class)) != null) {
                arrayList.add(uid);
            }
        }
        notifyResponseHandlers(responseArrFetch);
    ...
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:14:0x0028 -> B:15:0x0029). Please report as a decompilation issue!!! */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public synchronized void idleStart() throws ProtocolException {
        Response responseByeResponse;
        Response responseByeResponse2;
        ...
        while (!z) {
            try {
                ...
                responseByeResponse2 = Response.byeResponse(e3);
            }
            responseByeResponse2 = readResponse();
            arrayList.add(responseByeResponse2);
            if (!responseByeResponse2.isContinuation() && !responseByeResponse2.isBYE()) {
            }
            z = true;
            while (!z) {
            }
    ...
    public int[] uidfetchChangedSince(long j, long j2, long j3) throws ProtocolException {
        ...
        int length = responseArrCommand.length;
        for (int i = 0; i < length; i++) {
            if (responseArrCommand[i] != null && (responseArrCommand[i] instanceof FetchResponse)) {
                arrayList.add(Integer.valueOf(((FetchResponse) responseArrCommand[i]).getNumber()));
            }
        }
        notifyResponseHandlers(responseArrCommand);
        handleResult(responseArrCommand[responseArrCommand.length - 1]);
